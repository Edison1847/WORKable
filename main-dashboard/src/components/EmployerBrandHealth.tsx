...
const EmployerBrandHealth: React.FC = () => {
  ...
  useEffect(() => {
    fetch('http://localhost:3000/api/culture/employer-brand')
      .then(res => res.json())
      .then(result => {
        if (result.cycles) {
          setData(result.cycles);
          setGap(result.gap);
        }
      })
      .catch(err => console.error('Failed to fetch employer brand data:', err));
  }, []);

  return (
    <div className="rounded-2xl overflow-hidden" style={{ background: 'var(--bg-card)', border: '1px solid var(--border-1)', borderLeft: '2px solid #38bdf8', boxShadow: 'var(--shadow-md)' }}>
      ...
      <div className="p-5">
        <div className="grid grid-cols-3 gap-3 mb-4">
          {[
            ...
            <div key={k.label} className="rounded-xl p-3 text-center" style={{ background: `${k.color}08`, border: `1px solid ${k.color}1a` }}>
        ...
        <div className="flex items-start gap-2 mt-4 rounded-xl p-3" style={{ background: 'rgba(251,146,60,0.06)', border: '1px solid rgba(251,146,60,0.18)' }}>
